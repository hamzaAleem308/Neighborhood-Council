...
export default function NominatePanel ({route, navigation}) {
  ...
  const setUpPanel = async() => {
    ...
    const payload = { 
      ...
      panelMembersId: selectedMembers.map((member) => member.value),
      PanelName : panel,
      councilId : councilId,
      MemberId : memberId
    }
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.setText}>
        ...
        <Dropdown
          data={membersName.filter((member) => member.value !== candidateId && !selectedMembers.some((m) => m.value === member.value))} // Exclude chairman and already selected members
          style={styles.dropDown}
          maxHeight={200}
          labelField="label"
          valueField="value"
          ...
          onChange={(item) => addPanelMember(item)} // Add member to selected list
          renderPlaceholder={() => (
